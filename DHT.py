...
class sensor:
   ...
   def __init__(self, pi, gpio, model=DHTAUTO, callback=None):
      ...
      #pigio daemon check rising edge on pin gpio (interrupt triggered on rising edge of clock, get sensor reading)
   ...
   def _decode_dhtxx(self):
      """
      function is used to write self._temperature and self._humidity
            +-------+-------+
            | DHT11 | DHTXX |
            +-------+-------+
      Temp C| 0-50  |-40-125|
      ...
      #take individual bytes and create a checksum
      chksum = (b1 + b2 + b3 + b4) & 0xFF
      
      #if chksum is correct
      if chksum == b0:
        ...
        #check if temperature and humidity are within range
         ...
         #if temperature and humidity are valid, overwrite the temperature and humidity 
         if valid:
            self._temperature = t
            self._humidity = h
   ...
   def _rising_edge(self, gpio, level, tick):
      """encapsulates message taken from DHT11 to a variable self._code"""
      
      edge_len = pigpio.tickDiff(self._last_edge_tick, tick)
      
      #record time of new recorded reading
      ...
      if edge_len > 10000:
         ...
         if self._in_code:
            ...
            #if 40 bits is read, the length of the DHT11 message
            if self._bits == 40:
            
               #convert the code bit to 5 bytes
               self._decode_dhtxx()
   ...
   def _trigger(self):
      """Function used to wake up DHT sensor to initiate a reading."""
   ...
   def read(self):
      ...
      #host sends handshake message to start reading
      self._trigger()
      
      for i in range(5): # timeout after 0.25 seconds.
         time.sleep(0.05)
         
         #if decode_dhtxx is called and we get a successful reading
         if self._new_data:
            break
      
      #return reading in human readable format
      datum = self._datum()
